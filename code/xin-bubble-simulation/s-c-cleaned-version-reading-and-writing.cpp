...
bool ReadJson()
{
    ...
    //if input file does not exist, return false
    if (!file.is_open()) {
        return false;
    }
    ...
    mx=jsonFile["mx"];
    my=jsonFile["my"];
    omega=jsonFile["omega"];
    mstep=jsonFile["mstep"];
    cas=jsonFile["cas"];
    fcas=jsonFile["fcas"];
    bc=jsonFile["bc"];
    eos=jsonFile["eos"];
    dbcas=jsonFile["dbcas"];
    R=jsonFile["R"];
    A=jsonFile["A"];
    B=jsonFile["B"];
    T=jsonFile["T"];
    ...
    ly=jsonFile["ly"];
    c=jsonFile["c"];
    rho_l=jsonFile["rho_l"];
    rho_g=jsonFile["rho_g"];
    radius=jsonFile["radius"];
    freq=jsonFile["freq"];
    g=jsonFile["g"];
    if_th=jsonFile["if_th"];
    path=jsonFile["path"];
    return true;        
}
...
f[2][0][0] =f[4][0][0];
f[5][0][0] =f[7][0][0];
f[6][0][0]=1.0/2.0*(rho[0][0]-(f[0][0][0]+f[1][0][0]+f[2][0][0]+f[3][0][0]+f[4][0][0]+f[5][0][0]+f[7][0][0]));
f[8][0][0]=f[6][0][0];
...
f[3][i][j] =f[1][i][j];
f[6][i][j]=f[8][i][j];
f[7][i][j] =1.0/2.0*(rho[i][j]-(f[0][i][j]+f[1][i][j]+f[2][i][j]+f[3][i][j]+f[4][i][j]+f[6][i][j]+f[8][i][j]));
f[5][i][j] =f[7][i][j];
//Top left
i=0; j=my-1;
rho[i][j]=rho_l;
...
f[4][i][j] =f[2][i][j];
f[7][i][j] =f[5][i][j];
f[6][i][j]=1.0/2.0*(rho[i][j]-(f[0][i][j]+f[1][i][j]+f[2][i][j]+f[3][i][j]+f[4][i][j]+f[5][i][j]+f[7][i][j]));
f[8][i][j]=f[6][i][j];
}

void Non_Equilibrium_Bounce_Back2(double ***f,double **ux,double **uy,double **Fx,double **Fy,double **rho)
{
//west
    for (int j=1;j<my-1;j++)
    {
        ...
        f[5][0][j]=f[7][0][j]-1.0/2.0*(f[2][0][j]-f[4][0][j])+1.0/6.0*rho[0][j]*ux[0][j];
        f[8][0][j]=f[6][0][j]-1.0/2.0*(f[2][0][j]-f[4][0][j])+1.0/6.0*rho[0][j]*ux[0][j];
    }
//east
for (int j=1;j<my-1;j++)
    {
        ...
        uy[mx-1][j]=0.0;
        ux[mx-1][j]=(f[0][mx-1][j]+f[2][mx-1][j]+f[4][mx-1][j]+2.0*(f[1][mx-1][j]+f[5][mx-1][j]+f[8][mx-1][j]))/rho[mx-1][j]-1;
        f[3][mx-1][j]=f[1][mx-1][j]-2.0/3.0*rho[mx-1][j]*ux[mx-1][j];
        f[6][mx-1][j]=f[8][mx-1][j]-1.0/2.0*(f[2][mx-1][j]-f[4][mx-1][j])-1.0/6.0*rho[mx-1][j]*ux[mx-1][j];
        f[7][mx-1][j]=f[5][mx-1][j]+1.0/2.0*(f[2][mx-1][j]-f[4][mx-1][j])-1.0/6.0*rho[mx-1][j]*ux[mx-1][j];
    }
//north
for (int i=1;i<mx-1;i++)
    {
        ...
        ux[i][my-1]=0.0;
        uy[i][my-1]=(f[0][i][my-1]+f[1][i][my-1]+f[3][i][my-1]+2.0*(f[2][i][my-1]+f[5][i][my-1]+f[6][i][my-1]))/rho[i][my-1]-1;
        f[4][i][my-1]=f[2][i][my-1]-2.0/3.0*rho[i][my-1]*uy[i][my-1];
        f[7][i][my-1]=f[5][i][my-1]+1.0/2.0*(f[1][i][my-1]-f[3][i][my-1])-1.0/6.0*rho[i][my-1]*uy[i][my-1];
        f[8][i][my-1]=f[6][i][my-1]+1.0/2.0*(f[3][i][my-1]-f[1][i][my-1])-1.0/6.0*rho[i][my-1]*uy[i][my-1];
    }
//south
for (int i=1;i<mx-1;i++)
    {
        ...
        ux[i][0]=0.0;
        uy[i][0]=1-(f[0][i][0]+f[1][i][0]+f[3][i][0]+2.0*(f[4][i][0]+f[7][i][0]+f[8][i][0]))/rho[i][0];
        f[2][i][0]=f[4][i][0]+2.0/3.0*rho[i][0]*uy[i][0];
        f[5][i][0]=f[7][i][0]-1.0/2.0*(f[1][i][0]-f[3][i][0])+1.0/6.0*rho[i][0]*uy[i][0];
        f[6][i][0]=f[8][i][0]+1.0/2.0*(f[1][i][0]-f[3][i][0])+1.0/6.0*rho[i][0]*uy[i][0];
    }
//corner
// Bottom left (inlet)
rho[0][0]=rho_l;
...
void VerifyLapalaceLaw(std::string outputfile,double **press,double **rho,double *x)
{
...
{
//main loop
//if input.json does not exist,print error message
if(!ReadJson())
{
    std::cout<<"Error: input.json does not exist"<<std::endl;
    return 0;
    exit(0);
}
int *cx = new int[n]; 
...
int time=0;
std::string filename = path+std::string("animation") +std::to_string(time)+std::string(".dat");
std::string fcas_str="";
if(fcas==1) fcas_str="VSM";
else if(fcas==2) fcas_str="EDM";
else if(fcas==3) fcas_str="Guo";
std::string outputfile=path+"pressure_"+to_string_with_precision(lx,0)+"_"+std::to_string(mx)+"_"+to_string_with_precision(rho_l,2)+"_"+to_string_with_precision(radius,0)+"_"+fcas_str+".dat";
std::ofstream out;
...
for (time=1;time<mstep+1;++time)
{
    ...
    VerifyLapalaceLaw(outputfile,press,rho,x);//output radius,pressure and surface tension
    std::string filename = path+std::string("animation") +std::to_string(time)+std::string(".dat");
